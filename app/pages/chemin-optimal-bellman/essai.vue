   ...
   <v-row>
     ...
     <v-col cols="5" class="d-flex align-center justify-center px-4">
       <v-container>
         ...
         <v-container v-if="submit && choix==='Floyd Warshall'" class="bg-red">
           <p>Résultat de l'algorithme : ({{ choix }})</p>
           <v-list density="compact" class="bg-teal-lighten-4 rounded-xl w-100">
           <v-list-item
             v-for="(value, index) in res"
             :key="index"
           >
             <v-list-item-title>
                 {{ villes[index] }}
                 <v-row>
                   <p v-for="(villeCol, index) in value">
                   {{ villeCol }}
                 </p>
                 </v-row>
             </v-list-item-title>
